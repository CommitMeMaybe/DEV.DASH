...
function MetricCard({ icon, label, value, color }) {
  return (
    <div className="metric-card card">
      ...
      <div className="metric-value retro-text glow-text" style={{ color }}>
        {value}
      </div>
    </div>
  );
      ...
      <div className="bento-layout">
        <div className="bento-main">
          <div className="card span-full relative overflow-hidden">
            ...
            <div className="chart-container"><Skeleton height="160px" /></div>
          </div>
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="dashboard-container fade-in" style={{ textAlign: "center", padding: "4rem" }}>
...
export default function Dashboard() {
  const [githubUser, setGithubUser] = useState(() => (localStorage.getItem("devdash_github_user") || "").replace(/\s+/g, ""));
  const { user, repos, metrics, loading, error, lastSync, reload } = useGitHub(githubUser);
  const { tasks, toggleTask, removeTask } = useTasks();
  const { weather, loading: weatherLoading, city } = useWeather();
  const [showUsernameInput, setShowUsernameInput] = useState(false);
  const [tempUsername, setTempUsername] = useState(githubUser);
  const [avatarError, setAvatarError] = useState(false);

  const handleSetUsername = () => {
    setShowUsernameInput(true);
    setTempUsername(githubUser);
  };

  const handleUsernameSubmit = () => {
    if (tempUsername?.trim()) {
      const username = tempUsername.trim().replace(/\s+/g, "");
  ...
  return (
    <div className="dashboard-container fade-in">
      <header className="page-header">
        <div>
          ...
          <p className="text-muted">
            {user
              ? `Tracking: ${user.login} • ${repos?.length || 0} repos`
              : "System nominal."}
            {lastSync && " • Recently synced"}
          </p>
        </div>
        <div className="header-actions">
          {showUsernameInput ? (
            <div className="username-input-container card">
              <input
                type="text"
                value={tempUsername}
            ...
            <button
              className="card btn-retro user-button"
            ...
            >
              ...
              <span className="username-text">{githubUser}</span>
            </button>
          )}
      ...
      <div className="metrics-grid">
        <MetricCard
          icon={<Flame size={18} color="var(--color-warning)" />}
          label="STREAK"
          value={`${metrics?.streak || 0} DAYS`}
          color="var(--color-warning)"
        />
        <MetricCard
          icon={<GitCommit size={18} color="var(--text-accent)" />}
          label="COMMITS (7D)"
          value={metrics?.commitsThisWeek || 0}
          color="var(--text-accent)"
        />
        <MetricCard
          icon={<GitMerge size={18} color="var(--color-success)" />}
          label="PR MERGE RATE"
          value={`${metrics?.mergeRate || 0}%`}
          color="var(--color-success)"
        />
        <MetricCard
          icon={<Star size={18} color="var(--color-warning)" />}
          label="STARS (30D)"
          value={`+${metrics?.starsGained || 0}`}
          color="var(--color-warning)"
        />
      </div>
      ...
      <div className="bento-layout">
        <div className="bento-main">
          <div className="card span-full relative overflow-hidden">
            <div className="card-header">
              <h3 className="card-title">
                <Activity size={18} /> ACTIVITY SNAPSHOT
              </h3>
            </div>
          ...
          <div className="card" style={{ gridColumn: "1 / 2" }}>
            <div className="card-header">
              <h3 className="card-title">
                <TerminalSquare size={18} /> TOP REPOSITORY
              </h3>
            </div>
            {metrics?.topRepo ? (
              <div className="repo-details">
                <div className="repo-name retro-text">
                  {metrics.topRepo.full_name}
                </div>
                <div
                ...
                >
                  ...
                  <span
                    style={{
                      color: metrics.topRepo.language
                        ? "var(--text-accent)"
                        : "var(--text-secondary)",
                    }}
                  >
          ...
          <div className="card" style={{ gridColumn: "1 / -1" }}>
            ...
            {metrics?.activityByDate ? (
              <ContributionHeatmap data={metrics.activityByDate} />
            ) : (
        ...
        <div className="bento-side">
          <div className="card todo-card">
            <div className="card-header">
              <h3 className="card-title">
                <CheckCircle2 size={18} /> ACTIVE TASKS
              </h3>
            </div>
          ...
          <div className="card weather-mini-card">
            {weatherLoading ? (
              ...
              <div className="weather-mini-content">
                <span className="weather-temp retro-text glow-text">
                  {Math.round(weather.main.temp)}°C
                </span>
                <span className="weather-desc text-muted">
